...
def registrar_mercancia(request):
    ...
    muni = Municipio.objects.select_related(
        'dep_id').filter(mun_estado=1).order_by('mun_id')
    ...
    if request.method == "POST":
        ...
        emp = Empleado.objects.select_related(
            'suc_id').filter(per_documento=empleado)
        ...
        print(precio)
        try:
            person = Persona(per_documento=int(doc), td_id_id=1, per_nombre=nom, per_apellido=ape,
                             per_celular=cel, per_telefono=tel, per_email=email, per_direccion=direccion)
            person.save()
            ...
            if (existe):
                ...
                cliente = Cliente(per_documento_id=doc,
                                  cli_fecha_registro=fecha)
                cliente.save()
                cli = Cliente.objects.filter(per_documento=doc)

            destino = Destinatario(des_documento=desDoc, des_nombre=desNom, des_celular=desCel,
                                   des_telefono=desTel, des_direccion=desDir, des_email=desCor, mun_id_id=int(desMun))
            destino.save()
            numGuia = time.strftime("%Y%m%d") + str(randrange(10000))

            if (int(tm) == 1):
                mercancia = Mercancia(tm_id_id=int(tm), fp_id_id=int(fp), cli_documento_id=int(cli[0].cli_id), des_documento_id=int(desDoc), emp_id_id=int(
                    emp[0].emp_id), mer_contenido=con, mer_peso=int(peso), mer_precio=int(declarado), mer_precio_envio=int(precio), mer_num_guia=numGuia)
                mercancia.save()
                messages.success(
                    request, 'Registro exitoso. Número de Guía: ' + numGuia)
            else:
                mercancia = Mercancia(tm_id_id=int(tm), fp_id_id=int(fp), cli_documento_id=int(cli[0].cli_id), des_documento_id=int(desDoc), emp_id_id=int(emp[0].emp_id), mer_contenido=con, mer_peso=int(
                    peso), mer_alto=int(alto), mer_ancho=int(ancho), mer_largo=int(largo), mer_precio=int(declarado), mer_precio_envio=int(precio), mer_num_guia=numGuia)
                mercancia.save()
...
def buscar_cliente_ajax(request):
    ...
    data = serializers.serialize('json', person, fields=(
        'per_documento', 'per_nombre', 'per_apellido', 'per_telefono', 'per_celular', 'per_direccion', 'per_email'))
...
def buscar_destinatario_ajax(request):
    ...
    data = serializers.serialize('json', destinatario, fields=(
        'des_documento', 'mun_id', 'des_nombre', 'des_telefono', 'des_celular', 'des_direccion', 'des_email'))
    return HttpResponse(data, content_type='application/json')
...
def buscar_empleado_ajax(request):
    ...
    data = serializers.serialize(
        'json', person, fields=('emp_id', 'emp_estado'))
    return HttpResponse(data, content_type='application/json')
...
def buscar_precio(request):
    ...
    data = serializers.serialize('json', precio, fields=(
        'tm_id', 'tm_precio', 'tm_precio_adicional'))
    return HttpResponse(data, content_type='application/json')
...
def buscar_empleado(request):
    ...
    emp = Empleado.objects.filter(
        per_documento_id=documento).filter(emp_estado=1)
    if emp.exists() and emp[0].emp_estado == 1:
        tiposDoc = Tipo_documento.objects.all()
        roles = Rol.objects.all()
        sucursales = Sucursal.objects.all()
        per = emp[0].per_documento
        fin_contrato = str(emp[0].emp_fin_contrato)
        salario = int(emp[0].emp_salario)
        return render(request, 'eliminar_empleado.html', {'tipos': tiposDoc, 'roles': roles, 'suc': sucursales, 'per': per, 'emp': emp[0], 'fin_contrato': fin_contrato, 'salario':salario})
    else:
...
def l_envios(request):
    ...
    if usuario.is_active:
        ...
        mer_env = Mercancia_envio.objects.all()
        return render(request, 'l_envios.html', {'envios':envios, 'mer_env':mer_env})
    else:
        return redirect(reverse_lazy('login'))  # Probaaaaaar!!!
